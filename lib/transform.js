            ...
            if(node.name === variableName){
                ...
                if(
                  ...
                  (parent.type !== 'MemberExpression' || parent.object === node) && //y.x, or x.y
                   parent.key !== node) // { x: ... }
                {
                    transformations.push({ position: node.range[0], insert: '(' });
...
var _handleSync = function(node, content, asyncblockVarName, transformations){
    ...
    if(grandparent && grandparent.type === 'CallExpression' && block){
        ...
        //It's possible for a addAndReuseFiber to be inserted at the same position as a variable replacement. Make sure this one happens last.
        transformations.push({ position: prevCallEnd - 1, insert: flowInsertion, priority: 1 });
